...
namespace DTcms.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void user_oauth_bind(HttpContext context)
        {
            ...
            oauthModel.naem = dic["nick"].ToString();
            oauthModel.Image = dic["avatar"].ToString();
            oauthModel.gender = dic["sex"].ToString();
        ...
        private void cart_goods_update(HttpContext context)
        {
            ...
            Model.cart_keys model = Web.UI.ShopCart.Update(article_id, quantity);
            if (model != null)
            {
                context.Response.Write("{\"status\":1, \"msg\":\"商品数量修改成功！\", \"article_id\":" + model.article_id + ", \"quantity\":" + model.quantity + "}");
                return;
            }
        ...
        private void order_save(HttpContext context)
        {
            ...
            {
                //检查库存数量
                if (new BLL.article().GetStockQuantity(item.article_id) < item.quantity)
                {
        ...
        public static void Writr(Dictionary<string, object> dic)
        {
            if (dic != null)
            {
                string LogFileName = "QQLOG00001.txt";
                try
                {
                    foreach (KeyValuePair<string, object> pair in dic)
                    {
                        string WriteLogValue = pair.Key + " : " + pair.Value;
                        string m_LogName = HttpContext.Current.Server.MapPath(LogFileName);
                        ...
                        using (System.IO.FileStream fs = System.IO.File.Open(m_LogName, System.IO.FileMode.Append, System.IO.FileAccess.Write))
                        {
                            ...
                            {
                                w.WriteLine("【记录时间:{0}", DateTime.Now.ToString("yyyy-MM-dd-HH:mm:ss 记录信息：】    ") + WriteLogValue);
                                w.Flush();
                                w.Close();
                            }
                        }
